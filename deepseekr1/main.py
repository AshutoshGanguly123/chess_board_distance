...
class GraphConstructionFunctor:
    ...
    def _build_strategic_graph(self, position):
        ...
        board = position.board
        # Define central squares as strategic nodes
        central_squares = [chess.D4, chess.D5, chess.E4, chess.E5]
        for sq in central_squares:
            if board.piece_at(sq):
                G.add_node(sq, role="central")
        # Add kings as strategic milestones
        for color in [chess.WHITE, chess.BLACK]:
            king_sq = board.king(color)
            if king_sq is not None:
                G.add_node(king_sq, role="king")
        # Connect nodes that are spatially close (neighbors on board grid)
        nodes = list(G.nodes())
...
def algebraic_control_score(graph):
    """Compute algebraic connectivity (2nd smallest eigenvalue) plus max absolute Fiedler value."""
...
def algebraic_threat_score(graph):
    """Compute spectral radius (largest eigenvalue) plus max eigenvector centrality."""
    if graph.number_of_nodes() == 0:
        return 0.0
    try:
...
def algebraic_strategic_score(graph):
    """Compute smallest eigenvalue of normalized Laplacian plus max betweenness centrality.
       If the graph is directed, convert to undirected first.
    """
    if graph.number_of_nodes() == 0:
        return 0.0
    try:
...
def algebraic_hybrid_score(graphs):
    """Compute spectral radius of the tensor product of graphs.
       Convert all graphs to undirected to ensure symmetry.
    """
    try:
        for g in graphs:
...
def normalize_eval(eval_result):
    """Normalize Stockfish evaluation to roughly [-1, 1]. For centipawn (cp) values, scale by 100."""
    try:
...
def is_good_position(position, min_pieces=5):
    """
    Check if a position is "good" for analysis:
      - The board should not be in a game-over state.
      - It should have at least 'min_pieces' on board.
      - Its control graph should not be empty.
    """
    board = position.board
...
def generate_random_position(depth=10):
    """Generate a random chess position by playing a series of random legal moves."""
    board = chess.Board()
...
def generate_random_positions(num_positions=50, depth=10, max_attempts=200):
    """Generate a list of random chess positions that pass the is_good_position filter."""
    positions = []
    attempts = 0
...
def compute_similarity_metrics(position1, position2):
    """Compute absolute differences of the algebraic scores and evaluation between two positions."""
    try:
...
def visualize_positions(position1, position2):
    """Return two chess boards as SVG strings."""
    svg1 = chess.svg.board(position1.board, size=300)
    svg2 = chess.svg.board(position2.board, size=300)
    return svg1, svg2

# ---------------------------
# Experiment, Visualization, and Saving Comparisons
# ---------------------------
def run_experiments(num_positions=50, depth=10, num_sample_pairs=200):
    """Generate random positions, compute similarity metrics on random pairs, visualize results,
       and save each comparison's boards and metrics into separate folders.
    """
    # Base output directory (change if necessary)
    ...
    comparisons = []  # List to store (pos1, pos2, metrics) for each valid pair
    total_positions = len(positions)
    
    # Randomly sample pairs from the positions
    ...
    # Compute average differences for each metric and print them
    avg_metrics = {key: np.mean(vals) if len(vals) > 0 else 0.0 for key, vals in metrics.items()}
    print("Average Metric Differences Across Sample Pairs:")
    for key, avg in avg_metrics.items():
        print(f"{key}: {avg:.4f}")
    
    # Plot histograms of each metric difference
    ...
    # Save each comparison into its own folder
    for idx, comp in enumerate(comparisons):
        folder_name = os.path.join(base_output_dir, f"comparison_{idx+1}")
    ...
    # Visualize one pair of boards from the comparisons (for interactive inspection)
...
# ---------------------------
# Update this path to point to your Stockfish executable (for macOS, adjust as needed)
